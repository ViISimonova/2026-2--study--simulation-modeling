```@meta
EditURL = "../scripts/sir_optimize_with_constraint_literate.jl"
```

# Optimization with a Peak Constraint

This script implements the sixth additional task from the lab:
minimize mortality while keeping the epidemic peak below 30 percent.

````@example sir_optimize_with_constraint_literate
using DrWatson
