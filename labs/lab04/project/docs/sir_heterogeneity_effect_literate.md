```@meta
EditURL = "../scripts/sir_heterogeneity_effect_literate.jl"
```

# Heterogeneity Effect in the Agent-Based SIR Model

This script studies the third additional task from the lab:
different cities use different infection parameters, and the epidemic
dynamics are tracked separately for each city.

````@example sir_heterogeneity_effect_literate
using DrWatson
...
Helper function to count agents with a given state inside one city.
...
Heterogeneous infection parameters for the three cities.

````@example sir_heterogeneity_effect_literate
    params = Dict(
        :Ns => [1000, 1000, 1000],
        :beta_und => [0.3, 0.5, 0.8],
        :beta_det => [0.03, 0.05, 0.08],
        :infection_period => 14,
        :detection_time => 7,
        :death_rate => 0.02,
...
Manual simulation loop with per-city data collection.
...
Save the full per-city dynamics.
...
Build three panels for infected, susceptible, and recovered agents.
