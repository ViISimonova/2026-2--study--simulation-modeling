```@meta
EditURL = "../scripts/sir_quarantine_effect_literate.jl"
```

# Quarantine Effect in the Agent-Based SIR Model

This script implements the fifth additional task from the lab:
compare a baseline epidemic scenario with a scenario where migration
from a city is blocked after the infected fraction exceeds a threshold.

````@example sir_quarantine_effect_literate
using DrWatson
